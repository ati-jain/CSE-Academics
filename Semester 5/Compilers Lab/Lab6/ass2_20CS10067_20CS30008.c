...
int printFloat(float f)
{
	...
	else
	{
		...
		while (f != (int)f)
		{ // Checking if the decimal point is reached in our iteration
			f *= 10;
...
int readInt(int *ep)
{
	...
	if (len > 9 || len == 0)
	{ // Less than 9 bits allowed, keeping in mind the range of int in C
		*ep = 1;
		return 0;
	}
	if (n[0] == '-')
	{
		...
		{
			if (n[i] == '-')
				*ep = 1; // A number can contain '-' only at the starting of the number
			num *= 10;
			num += (int)n[i] - '0';
		}
		num = -num;
...
int readFloat(float *ep)
{
	...
	float mul = 1.0;
	if (len > 12 || len == 0)
	{ // Float is limited to 12 bits to avoid overflow
		*ep = 0.0;
		return 1;
	}
	if (n[0] == '.')
	...
	if (n[0] == '-')
	{
		...
		{
			if (n[i] == '-')
				return 1; // A number can contain '-' only at the starting of the number
			if (n[i] == '.' && flag == 1)
				return 1; // A number can contain '.' only once
			if (n[i] == '.' && flag == 0)
			{
				flag = 1;
				continue;
