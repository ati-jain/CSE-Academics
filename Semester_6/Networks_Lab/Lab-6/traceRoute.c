...
void send_packet(int sockfd, char *ip_addr, char *data, int ttl);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < 255; i++)
    {
        send_packet(sockfd, ip_addr, msg, i);
        sleep(1);
        printf("Packet Sent!\n");

        ip = receive_packet(sockfd);
        sleep(1);
        if (ip < 0)
        {
            printf("Some error\n");
...
void setIP(struct iphdr *ip_hdr, char *ip_addr, int ttl)
{
    ip_hdr->ihl = 5;
    ip_hdr->version = 4;
    ip_hdr->tos = 0;
...
void send_packet(int sockfd, char *ip_addr, char *data, int ttl)
{
    ...
    setIP(ip_hdr, inet_ntoa(*((struct in_addr *)he->h_addr)), ttl);
